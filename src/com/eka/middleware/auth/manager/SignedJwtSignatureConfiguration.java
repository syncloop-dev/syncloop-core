...
public class SignedJwtSignatureConfiguration extends AbstractSignatureConfiguration {
	...
	public SignedJwtSignatureConfiguration(final String secret) {
        this(secret.getBytes(UTF_8));
    }

	public SignedJwtSignatureConfiguration(final byte[] secret) {
        this();
        this.secret = Arrays.copyOf(secret,secret.length);
    }

	public SignedJwtSignatureConfiguration(final String secret, final JWSAlgorithm algorithm) {
        this(secret.getBytes(UTF_8),algorithm);
    }

	public SignedJwtSignatureConfiguration(final byte[] secret, final JWSAlgorithm algorithm) {
        this.secret = Arrays.copyOf(secret,secret.length);
	...
	public boolean verify(final SignedJWT jwt) throws JOSEException {
		...
		//KeyPair kp= JWKHelper.buildECKeyPairFromJwk("");
		//JWKHelper.buildSecretFromJwk(jwt.getSigningInput())
